...
- The latest sequential full-suite validation passed at `1548 passed, 4 skipped`.
...
- `app.py` now delegates chart-query validation, SQL literal/coercion helpers, default/raw chart-spec assembly, chart-spec normalization, builder SQL compilation, compiled-spec validation, template role-map resolution, boolean parsing, and chart visual override handling to `shared/chart_specs.py` while preserving the app-level wrappers exercised by dashboard chart compile, validate, render, named-query, and non-SELECT rejection regressions.
