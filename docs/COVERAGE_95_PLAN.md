...
Status: In progress, with dependency parsing, GitHub contents inspection, onboarding repository-readiness checks, onboarding issue-body builders, onboarding work-item persistence, shared onboarding issue-result orchestration, and GitHub repo import/list lookup helpers extracted in this phase.
...
- `app.py` now delegates the onboarding issue-body formatting, work-item persistence, repeated onboarding issue result handling, and GitHub repo import/list lookup paths to `shared/onboarding.py`, leaving a smaller route/orchestration surface behind.
