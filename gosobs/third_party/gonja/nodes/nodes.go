...
// Condition is a parenthesized inline conditional expression used as a value:
// (Expression if Condition else Alternative). PORT-NOTE (SOBS): gonja v2.8.0
// only parses inline conditionals at the top level of {{ }} / {% set %} (via the
// Output node), not nested inside parentheses. Several templates use
// `(a if c else b)` as a sub-expression, so this node type plus the parser and
// evaluator support restore that Jinja2 behaviour.
...
func (c *Condition) Position() *tokens.Token { return c.Location }
func (c *Condition) String() string {
	if c.Alternative != nil {
		return fmt.Sprintf("(%s if %s else %s)", c.Expression, c.Condition, c.Alternative)
	}
	return fmt.Sprintf("(%s if %s)", c.Expression, c.Condition)
}
