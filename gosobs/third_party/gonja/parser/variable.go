...
func (p *Parser) parseTupleOrExpression() (nodes.Expression, error) {
	...
	// PORT-NOTE (SOBS): ParseValueExpression allows inline conditionals as values,
	// e.g. (a if cond else b) — Jinja2 behaviour gonja v2.8.0 lacks.
	expression, err := p.ParseValueExpression()
	if err != nil {
		return nil, err
	}
		...
		if accessor := p.Match(tokens.Dot, tokens.LeftBracket); accessor != nil {
			...
			for p.Match(tokens.Comma) != nil || p.Match(tokens.RightParenthesis) == nil {
				...
				if p.Match(tokens.Assign) != nil {
					key := v.Position().Val
					value, errValue := p.ParseValueExpression()
					if errValue != nil {
						return nil, errValue
					}
