...
// evalCondition evaluates a parenthesized inline conditional (ternary)
// expression: (Expression if Condition else Alternative). PORT-NOTE (SOBS).
func (e *Evaluator) evalCondition(node *nodes.Condition) *Value {
	cond := e.Eval(node.Condition)
	if cond.IsError() {
		return AsValue(errors.Wrapf(cond, `Unable to evaluate condition %s`, node.Condition))
	}
	if cond.IsTrue() {
...
func (e *Evaluator) evalBinaryExpression(node *nodes.BinaryExpression) *Value {
	...
	case tokens.And:
		// PORT-NOTE (SOBS): Jinja `and`/`or` return the *operand value*, not a
		// boolean (`x or ''` => x if truthy else ''). Upstream gonja returned a
		// bool, which breaks templates like `(v or '') | trim` and
		// `(m or {}).get(...)`. Return operands to match Jinja2/Python semantics.
		if !left.IsTrue() {
			return left
		}
		right = e.Eval(node.Right)
...
func (e *Evaluator) evalGetSlice(node *nodes.GetSlice) *Value {
	...
	// PORT-NOTE (SOBS): clamp indices to [0, len] like Python slicing (`s[:80]`
	// on a shorter string yields the whole string). Upstream gonja passed the
	// raw bounds to a Go slice, panicking when end > len.
	n := value.Len()
	if start < 0 {
		start = 0
	}
