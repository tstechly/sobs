...
from shared.ai_trace_builder import _build_ai_trace_turn_cards as _shared_build_ai_trace_turn_cards
from shared.ai_trace_builder import _dedupe_system_input_messages as _shared_dedupe_system_input_messages
from shared.ai_trace_builder import _extract_messages_text as _shared_extract_messages_text
from shared.ai_trace_builder import _first_message_content as _shared_first_message_content
from shared.ai_trace_builder import _genai_message_content_to_text as _shared_genai_message_content_to_text
from shared.ai_trace_builder import _genai_message_reasoning_to_text as _shared_genai_message_reasoning_to_text
from shared.ai_trace_builder import _genai_tool_calls_to_text as _shared_genai_tool_calls_to_text
from shared.ai_trace_builder import _normalize_for_dedupe as _shared_normalize_for_dedupe
from shared.ai_trace_builder import (
    _normalize_genai_messages_for_display as _shared_normalize_genai_messages_for_display,
)
from shared.ai_trace_builder import _parse_genai_messages_json as _shared_parse_genai_messages_json
from shared.ai_trace_builder import _string_attr_truthy as _shared_string_attr_truthy
from shared.ai_trace_builder import _summarize_ai_tool_action as _shared_summarize_ai_tool_action
...
from shared.db_stats import _active_part_rows as _shared_active_part_rows
from shared.db_stats import _fmt_bytes as _shared_fmt_bytes
from shared.db_stats import _get_db_stats as _shared_get_db_stats
from shared.error_items import _build_error_item as _shared_build_error_item
from shared.error_items import _compact_text as _shared_compact_text
from shared.error_items import _error_group_key as _shared_error_group_key
from shared.error_items import _extract_structured_error_summary as _shared_extract_structured_error_summary
from shared.error_items import _try_pretty_json_text as _shared_try_pretty_json_text
...
def _rum_session_key_from_attrs(attrs: dict[str, str], ts: str, body_raw: str) -> str:
    return _shared_rum_session_key_from_attrs(attrs, ts, body_raw)


def _build_rum_event_item(row: Any) -> dict[str, Any]:
    return _shared_build_rum_event_item(row, map_to_dict=_map_to_dict)
...
def _proto_logs_to_events(msg: ExportLogsServiceRequest) -> list[LogEvent]:
    return _shared_proto_logs_to_events(msg, ns_to_iso=_ns_to_iso)


def _proto_traces_to_events(msg: ExportTraceServiceRequest) -> tuple[list[SpanEvent], list[ErrorEvent]]:
    return _shared_proto_traces_to_events(msg, ns_to_iso=_ns_to_iso)


def _proto_metrics_to_events(msg: ExportMetricsServiceRequest) -> list[TypedMetricEvent]:
    return _shared_proto_metrics_to_events(msg, ns_to_iso=_ns_to_iso, now_iso=_now_iso)
...
def _compact_text(value: str, limit: int = 220) -> str:
    return _shared_compact_text(value, limit)


@lru_cache(maxsize=4096)
def _try_pretty_json_text(raw_value: str) -> tuple[bool, str]:
    return _shared_try_pretty_json_text(raw_value)


def _extract_structured_error_summary(message: str, raw_body: str) -> tuple[str, bool]:
    return _shared_extract_structured_error_summary(message, raw_body)


def _build_error_item(row: dict) -> dict:
    return _shared_build_error_item(
        row,
...
def _error_group_key(item: dict) -> tuple[str, str, str]:
    return _shared_error_group_key(item)
...
def _active_part_rows(db, table_name: str) -> int:
    return _shared_active_part_rows(db, table_name)
...
def _get_db_stats(db) -> dict:
    return _shared_get_db_stats(db, log_debug=app.logger.debug)


def _fmt_bytes(n: int | None) -> str:
    return _shared_fmt_bytes(n)
